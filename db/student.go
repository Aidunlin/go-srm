...
func SelectStudent(id int) (model.StudentRecord, bool) {
	...
	sql := fmt.Sprintf("SELECT * FROM %v WHERE id = ? LIMIT 1", studentTableName)
...
func InsertStudent(record model.StudentRecord) bool {
	...
	sql := fmt.Sprintf("INSERT INTO %v (%v) VALUES (%v)", studentTableName, namesString, placeholdersString)
...
func UpdateStudent(id int, record model.StudentRecord) bool {
	...
	sql := fmt.Sprintf("UPDATE %v SET %v WHERE id = ?", studentTableName, settersString)
	_, execErr := db.Execute(sql, values...)
	return execErr == nil
}
...
func DeleteStudent(id int) bool {
	...
	sql := fmt.Sprintf("DELETE FROM %v WHERE id = ?", studentTableName)
	result, execErr := db.Execute(sql, id)
...
func selectStudentsWithWhere(db *client.Conn, params model.StudentTableParams, whereSql string) (int64, []model.StudentRecord) {
	totalSql := fmt.Sprintf("SELECT COUNT(*) AS total from %v %v", studentTableName, whereSql)
	...
	recordsSql := fmt.Sprintf("SELECT * FROM %v %v %v %v", studentTableName, whereSql, orderSql, pageSql)
